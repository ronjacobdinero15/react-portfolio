...
function SectionLink({ title, even = false }) {
  ...
  return (
    <div
      className={`group mb-3 flex items-center gap-2 ${even ? "flex-row-reverse" : ""} ${firstWord === "contact" ? "flex-row-reverse justify-center [&_div:after]:pb-0 [&_div:after]:text-5xl [&_div:after]:sm:text-6xl [&_div:after]:xl:text-8xl [&_div]:pb-[2px] [&_div]:text-5xl [&_div]:shadow-md [&_div]:sm:text-6xl [&_div]:xl:text-8xl" : ""}`}
    >
